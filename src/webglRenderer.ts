...
import { ForceGraph, projectPoint } from "./forceGraph.js";
...
function toColorFloats(bytes: Uint8Array, fallback: [number, number, number, number], itemCount: number): Float32Array {
...
export class WebGLForceGraphRenderer {
  ...
  private readonly aPointSize: number;
  private readonly uViewport: WebGLUniformLocation;
  private readonly uNodeShape: WebGLUniformLocation;
  private readonly uRenderMode: WebGLUniformLocation;
  ...
  private shapeToNumber(shape: DrawOptions["nodeShape"]): number {
    switch (shape) {
      case "square":
        return 1;
      case "diamond":
  ...
  private projectPositions(
    ...
    options: DrawOptions
  ): { positions: Float32Array; depthScale: Float32Array } {
  ...
  private expandEdgeGeometry(
    projected: { positions: Float32Array; depthScale: Float32Array },
    snapshot: RenderSnapshot,
  ...
  private bindAttributeBuffer(buffer: WebGLBuffer, location: number, data: Float32Array, size: number): void {
    const gl = this.gl;
